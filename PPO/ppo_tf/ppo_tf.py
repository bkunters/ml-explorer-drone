...
input_length_net = 8            # input layer size
policy_output_size = 2          # policy output layer size
discount_factor = 0.99
env_name = "LunarLander-v2"     # LunarLander-v2 or MountainCar-v0 or CartPole-v1 or Pendulum-v1
...
env = gym.make(env_name, continuous=continous)
...
def train():
    ...
    for epochs in range(num_total_steps):
        ...
        wandb.log({
            'train/time steps': num_passed_timesteps, 
            'train/policy loss': policy_loss, 
            'train/value loss': value_loss, 
            'train/mean return': mean_return})
